...
R can deal with a much wider variety of complex data types than the simple features standard, with key examples in `ggplot2` family, `igraph` networks, `rgl` indexed mesh models, and `adehabitatLT`, `trajectories` and `move` track models of objects moving in time. There is a need for a common language for translating and storing data between these specialized forms
