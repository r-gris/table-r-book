...
ggplot(tab) + aes(x = long, y = lat, group = feat, fill = id) + geom_holygon(col = "black")

## if we use geom_holygon, we can do it the old way
ggplot(tab) + aes(x = long, y = lat, group = group, fill = id) + geom_holygon(col = "black")

## use pathGrob and it's all fine
ggplot(filter(holey, object_ <= 2)) + 
  aes(x = x_, y = y_, group = branch_, fill = factor(object_)) + geom_holygon(col = "black")
